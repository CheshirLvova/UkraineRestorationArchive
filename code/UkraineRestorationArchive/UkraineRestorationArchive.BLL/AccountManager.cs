...
namespace UkraineRestorationArchive.BLL
{
    public class AccountManager : IAccountManager
    {
        ...
        public AccountManager(UkraineRestorationArchiveDBContext dBContext)
        {
            _db = dBContext;
        }
            ...
            strategy.Execute(
                () =>
                {
                    using (var trainsaction = _db.Database.BeginTransaction())
                    {
                        ...
                        _db.Users.Add(new User { Username = username, EmailAddres = emailAddres, Password = encryptedPassword, Type = "User" });
                        _db.SaveChanges();
                        trainsaction.Commit();
                    }
                });
